...
int main() {
    ...
    std::string import_file = {"E:\\dev_libraries\\tensorflow\\tensorflow\\opencv_tensorflow_video\\trained_model\\frozen_inference_graph.pb"};
    ...
    while (true) {
        ...
        if (!frame.empty()) {
            ...
            // cv::resize(processed_frame, processed_frame, cv::Size(frame.cols, frame.rows));
